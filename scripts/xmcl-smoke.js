...
(async () => {
  ...
  const requiredInstaller = ['getVersionList', 'install', 'installTask', 'installDependencies', 'installDependenciesTask', 'getLoaderArtifactListFor', 'installFabricByLoaderArtifact'];
  ...
  try {
    ...
    if (!task || typeof task.startAndWait !== 'function' || typeof task.cancel !== 'function') {
      throw new Error('@xmcl/installer installTask does not match the task-based API expected by Dih');
    }
  } finally {
